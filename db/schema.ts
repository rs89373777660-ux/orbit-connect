...
export const messages=sqliteTable("messages",{id:text("id").primaryKey(),chatId:text("chat_id").notNull(),senderId:text("sender_id").notNull(),body:text("body"),kind:text("kind",{enum:["text","file","photo","sticker","voice","system"]}).notNull(),fileKey:text("file_key"),fileName:text("file_name"),fileSize:integer("file_size"),fileMime:text("file_mime"),replyTo:text("reply_to"),forwardedFromId:text("forwarded_from_id"),editedAt:integer("edited_at"),deletedAt:integer("deleted_at"),createdAt:integer("created_at").notNull()},t=>[index("idx_messages_chat_created").on(t.chatId,t.createdAt)]);
