...
export const messagePins=sqliteTable("message_pins",{messageId:text("message_id").primaryKey(),chatId:text("chat_id").notNull(),pinnedBy:text("pinned_by").notNull(),scope:text("scope",{enum:["self","all"]}).notNull().default("all"),createdAt:integer("created_at").notNull()},t=>[index("idx_message_pins_chat_created").on(t.chatId,t.createdAt)]);
