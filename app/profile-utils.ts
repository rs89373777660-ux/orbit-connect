...
export function publicProfile(row:typeof users.$inferSelect,isOwn=false){
 ...
 return {
  ...
  avatarUrl:show(row.privacyPhoto&&Boolean(row.avatarData||row.avatarAssetId)&&!row.avatarPreset,(row.avatarData||row.avatarAssetId)&&!row.avatarPreset?`/api/avatar?id=${encodeURIComponent(row.id)}`:null),
  hasAvatar:Boolean(row.avatarData||row.avatarAssetId),
