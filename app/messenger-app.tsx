...
type Message={id:string;senderId:string;body:string|null;kind:string;fileName?:string|null;fileSize?:number|null;fileMime?:string|null;attachments?:MessageAttachment[];reactions?:MessageReaction[];replyTo?:string|null;forwardedFromId?:string|null;editedAt?:number|null;deletedAt?:number|null;deliveryStatus?:"sent"|"delivered"|"read"|"failed";failed?:boolean;createdAt:number};
...
export default function MessengerApp(){
 ...
 async function loadChats(){
  ...
  try{
   ...
   list.filter(chat=>chat.id!==current?.id).slice(0,12).forEach((chat,index)=>window.setTimeout(()=>void preloadMessages(chat.id),80+index*90));
 ...
 async function sendPendingAttachments(){
  ...
  try{const prepared=await Promise.all(batch.map(item=>item.kind==="photo"?compressPhoto(item.file):Promise.resolve(item.file))),form=new FormData();prepared.forEach(file=>form.append("files",file));form.set("chatId",chatId);form.set("caption",caption);const total=prepared.reduce((sum,file)=>sum+file.size,0)||1;
   ...
   if(!data.message)throw new Error(data.error||"Фотографии не отправлены");setMessages(current=>current.map(message=>message.id===temporaryId?{...data.message!,deliveryStatus:"delivered"}:message));batch.forEach(item=>{if(item.previewUrl)window.setTimeout(()=>URL.revokeObjectURL(item.previewUrl!),1200)});messageCache.current.delete(chatId);window.setTimeout(()=>void loadMessages(chatId),250);notify(batch.length===1?"Фотография отправлена":`Отправлено фотографий: ${batch.length}`)
...
function MessageBubble({message,mine,currentUserId,reply,selectionMode,selected,menu,select,toggle,answer,share,retry,seen,structured,react}:{message:Message;mine:boolean;currentUserId:string;reply?:Message;selectionMode:boolean;selected:boolean;menu:()=>void;select:()=>void;toggle:()=>void;answer:()=>void;share:()=>void;retry:()=>void;seen:()=>void;structured:(action:"poll-vote"|"checklist-toggle",index:number)=>void;react:(emoji:string)=>void}){
 const start=useRef({x:0,y:0}),timer=useRef<number|undefined>(undefined),longPressed=useRef(false),moved=useRef(false),suppressClick=useRef(false),bubbleRef=useRef<HTMLDivElement|null>(null),seenOnce=useRef(false);const [dragX,setDragX]=useState(0),[dragging,setDragging]=useState(false),[reactionOpen,setReactionOpen]=useState(false);
 ...
 function openMenu(){const rect=bubbleRef.current?.getBoundingClientRect();if(rect){const viewportHeight=window.innerHeight,width=Math.min(300,window.innerWidth-32),menuHeight=Math.min(370,viewportHeight-32),left=mine?Math.max(16,rect.right-width):Math.min(window.innerWidth-width-16,Math.max(16,rect.left)),visibleTop=Math.max(16,rect.top),visibleBottom=Math.min(viewportHeight-16,rect.bottom),centeredTop=(visibleTop+visibleBottom-menuHeight)/2,belowTop=rect.bottom+8,aboveTop=rect.top-menuHeight-8,preferredTop=rect.height>viewportHeight-32?centeredTop:(viewportHeight-rect.bottom>=menuHeight?belowTop:aboveTop),top=Math.max(16,Math.min(viewportHeight-menuHeight-16,preferredTop));document.documentElement.style.setProperty("--message-menu-left",`${left}px`);document.documentElement.style.setProperty("--message-menu-top",`${top}px`);document.documentElement.style.setProperty("--message-menu-bottom","auto")}menu()}
 ...
 return <div className={`message-choice-row${mine?" mine":""}${selected?" selected":""}`}>
  ...
  {!message.deletedAt&&<div className="message-reactions">{[...new Set((message.reactions||[]).map(item=>item.emoji))].map(emoji=>{const items=(message.reactions||[]).filter(item=>item.emoji===emoji),mineReaction=items.some(item=>item.userId===currentUserId);return <button key={emoji} className={mineReaction?"mine-reaction":""} onClick={()=>react(emoji)}>{emoji}<small>{items.length}</small></button>})}{!mine&&<button className="add-reaction" aria-label="Добавить реакцию" onClick={()=>setReactionOpen(value=>!value)}>☺＋</button>}{reactionOpen&&<div className="reaction-picker">{"👍 ❤️ 😂 😮 😢 🔥 👏 🎉 🤔 🙏".split(" ").map(emoji=><button key={emoji} onClick={()=>{react(emoji);setReactionOpen(false)}}>{emoji}</button>)}</div>}</div>}
