...
export default function MessengerApp(){
 ...
 const [galleryOpen,setGalleryOpen]=useState(false),[galleryItems,setGalleryItems]=useState<Array<{id:string;kind:string;fileName?:string;url:string}>>([]),[avatarEditor,setAvatarEditor]=useState<{dataUrl:string;zoom:number;x:number;y:number;enhance:boolean}|null>(null),[avatarGallery,setAvatarGallery]=useState<Array<{id:string;url:string;label:string}>>([]);
 ...
 async function uploadAvatar(event:ChangeEvent<HTMLInputElement>){const file=event.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>setAvatarEditor({dataUrl:String(reader.result),zoom:1,x:0,y:0,enhance:true});reader.readAsDataURL(file);event.target.value=""}
 async function saveAvatar(){if(!avatarEditor||!profile)return;const image=new Image();image.src=avatarEditor.dataUrl;await new Promise(resolve=>image.onload=resolve);const canvas=document.createElement("canvas"),size=512;canvas.width=size;canvas.height=size;const context=canvas.getContext("2d")!;if(avatarEditor.enhance)context.filter="contrast(1.08) saturate(1.14) brightness(1.03)";const base=Math.max(size/image.naturalWidth,size/image.naturalHeight)*avatarEditor.zoom,w=image.naturalWidth*base,h=image.naturalHeight*base;context.drawImage(image,(size-w)/2+avatarEditor.x*1.8,(size-h)/2+avatarEditor.y*1.8,w,h);const avatarData=canvas.toDataURL("image/jpeg",.86),r=await appFetch("/api/avatar",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({avatarData,label:avatarEditor.enhance?"ИИ-улучшение":"Оригинал"})}),data=await r.json().catch(()=>({}));if(r.ok){setProfile({...profile,avatarUrl:data.avatarUrl,avatarPreset:null,hasAvatar:true});setAvatarEditor(null);notify("Аватар сохранён")}else notify(data.error||"Фото не загружено")}
 ...
 return <main className="orbit-v4" style={{"--accent":accent} as CSSProperties}>
...
function Registration({profile,onComplete,notify}:{profile:Profile;onComplete:(profile:Profile)=>void;notify:(text:string)=>void}){
 const [step,setStep]=useState<"phone"|"login"|"reset"|"code"|"profile">("phone"),[phone,setPhone]=useState(""),[code,setCode]=useState(""),[password,setPassword]=useState(""),[confirm,setConfirm]=useState(""),[demoCode,setDemoCode]=useState(""),[busy,setBusy]=useState(false);
 const [form,setForm]=useState({name:"",email:"",birthYear:"",handle:profile.handle||profile.publicId||"",telegram:"",vk:"",website:""});
 async function requestCode(){setBusy(true);try{const r=await appFetch("/api/registration",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"request-code",phone})}),data=await r.json().catch(()=>({}));if(r.status===409){setStep("login");notify("Номер уже зарегистрирован — введите пароль")}else if(!r.ok)notify(data.error||"Не удалось отправить код");else{setDemoCode(data.demoCode||"");setStep("code")}}catch{notify("Нет связи с сервисом регистрации")}finally{setBusy(false)}}
 async function login(){setBusy(true);try{const r=await fetch("/api/auth",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"login",phone,password})}),data=await r.json().catch(()=>({}));if(!r.ok){notify(data.error||"Не удалось войти");return}localStorage.setItem("orbit_session",data.token);location.reload()}catch{notify("Нет связи с сервером входа")}finally{setBusy(false)}}
 async function requestReset(){setBusy(true);const r=await fetch("/api/auth",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"request-password-code",phone})}),data=await r.json().catch(()=>({}));setBusy(false);if(!r.ok)notify(data.error||"Код не отправлен");else{setDemoCode(data.demoCode||"");setStep("reset")}}
 async function resetPassword(){if(password!==confirm){notify("Пароли не совпадают");return}setBusy(true);const r=await fetch("/api/auth",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"reset-password",phone,code,password})}),data=await r.json().catch(()=>({}));setBusy(false);if(!r.ok)notify(data.error||"Пароль не установлен");else{localStorage.setItem("orbit_session",data.token);location.reload()}}
 ...
 {step==="phone"&&<><h1>Ваш номер</h1><p>Новый пользователь подтверждает телефон. Если аккаунт уже есть — войдите по паролю.</p><label>Номер телефона<input inputMode="tel" value={phone} onChange={event=>setPhone(event.target.value)} placeholder="+7 999 123-45-67"/></label><button disabled={busy||normalizePhone(phone).length<10} onClick={()=>void requestCode()}>{busy?"Отправляем…":"Получить код →"}</button><button className="link-button" disabled={normalizePhone(phone).length<10} onClick={()=>setStep("login")}>У меня уже есть аккаунт</button></>}
 {step==="login"&&<><h1>Вход по паролю</h1><p>Номер: +{normalizePhone(phone)}</p><label>Пароль<input type="password" value={password} onChange={event=>setPassword(event.target.value)} onKeyDown={event=>{if(event.key==="Enter")void login()}}/></label><button disabled={busy||password.length<6} onClick={()=>void login()}>Войти →</button><button className="link-button" onClick={()=>void requestReset()}>Задать или восстановить пароль по SMS</button><button className="link-button" onClick={()=>setStep("phone")}>Назад</button></>}
 {step==="reset"&&<><h1>Новый пароль</h1><p>Введите код из SMS и новый пароль.</p>{demoCode&&<div className="demo-code">Тестовый SMS-код: <b>{demoCode}</b></div>}<label>Код<input inputMode="numeric" maxLength={6} value={code} onChange={event=>setCode(event.target.value.replace(/\D/g,""))}/></label><label>Новый пароль<input type="password" value={password} onChange={event=>setPassword(event.target.value)}/></label><label>Повторите пароль<input type="password" value={confirm} onChange={event=>setConfirm(event.target.value)}/></label><button disabled={busy||code.length!==6||password.length<6||password!==confirm} onClick={()=>void resetPassword()}>Сохранить пароль и войти →</button></>}
...
function AvatarEditor({value,setValue,close,save}:{value:{dataUrl:string;zoom:number;x:number;y:number;enhance:boolean};setValue:(value:{dataUrl:string;zoom:number;x:number;y:number;enhance:boolean})=>void;close:()=>void;save:()=>void}){return <div className="modal-back"><div className="avatar-editor"><header><div><small>РЕДАКТОР АВАТАРА</small><h2>Выберите кадр</h2></div><button onClick={close}>×</button></header><div className="avatar-crop"><img src={value.dataUrl} alt="Новый аватар" style={{transform:`translate(${value.x}px,${value.y}px) scale(${value.zoom})`,filter:value.enhance?"contrast(1.08) saturate(1.14) brightness(1.03)":"none"}}/></div><label>Масштаб<input type="range" min="1" max="3" step=".05" value={value.zoom} onChange={event=>setValue({...value,zoom:Number(event.target.value)})}/></label><label>По горизонтали<input type="range" min="-80" max="80" value={value.x} onChange={event=>setValue({...value,x:Number(event.target.value)})}/></label><label>По вертикали<input type="range" min="-80" max="80" value={value.y} onChange={event=>setValue({...value,y:Number(event.target.value)})}/></label><label className="switch-row"><span><b>Автоулучшение ИИ</b><small>Свет, цвет и выразительность портрета</small></span><input type="checkbox" checked={value.enhance} onChange={event=>setValue({...value,enhance:event.target.checked})}/><i/></label><button className="primary-action" onClick={save}>Сохранить аватар</button></div></div>}
