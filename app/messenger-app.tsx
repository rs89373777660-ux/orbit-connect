"use client";
import { CSSProperties, ChangeEvent, FormEvent, Fragment, useEffect, useMemo, useRef, useState } from "react";
...
type Chat={id:string;name:string;kind:string;createdAt:number;lastActivityAt?:number;userId?:string|null;avatarUrl?:string|null;avatarPreset?:string|null;canPost?:boolean;pinnedAt?:number|null;systemPinned?:boolean;unreadCount?:number;online?:boolean;lastSeenAt?:number|null};
type ChatFolder={id:string;name:string;icon:string;chatIds:string[]};
...
function OrbitIcon({name,className=""}:{name:string;className?:string}){return <svg className={`orbit-glyph ${className}`} aria-hidden="true" focusable="false"><use href={`/orbit-ui-icons.svg?v=35#${name}`}/></svg>}
...
function dayLabel(value:number){const date=new Date(value),today=new Date(),yesterday=new Date(Date.now()-86400000);if(dayKey(value)===dayKey(today.getTime()))return "Сегодня";if(dayKey(value)===dayKey(yesterday.getTime()))return "Вчера";return date.toLocaleDateString("ru-RU",{day:"numeric",month:"long",year:date.getFullYear()===today.getFullYear()?undefined:"numeric"})}
...
export default function MessengerApp(){
 ...
 const [messageMenu,setMessageMenu]=useState<Message|null>(null),[pinMessageChoice,setPinMessageChoice]=useState<Message|null>(null),[editingMessage,setEditingMessage]=useState<Message|null>(null),[replyingTo,setReplyingTo]=useState<Message|null>(null),[deleteMessage,setDeleteMessage]=useState<Message|null>(null),[forwardMessages,setForwardMessages]=useState<Message[]>([]),[shareFallback,setShareFallback]=useState<{text:string;url:string}|null>(null),[selectedMessageIds,setSelectedMessageIds]=useState<Set<string>>(()=>new Set()),[batchDeleteOpen,setBatchDeleteOpen]=useState(false),[selectedChatIds,setSelectedChatIds]=useState<Set<string>>(()=>new Set()),[deleteChatsOpen,setDeleteChatsOpen]=useState(false),[chatFilter,setChatFilter]=useState("all"),[chatFolders,setChatFolders]=useState<ChatFolder[]>([]),[folderEditor,setFolderEditor]=useState(false),[folderName,setFolderName]=useState(""),[folderIcon,setFolderIcon]=useState("📁"),[aiMenuOpen,setAiMenuOpen]=useState(false),[emojiOpen,setEmojiOpen]=useState(false),[attachmentOpen,setAttachmentOpen]=useState(false),[richAttachment,setRichAttachment]=useState<RichAttachmentKind|null>(null),[pendingAttachments,setPendingAttachments]=useState<PendingAttachment[]>([]),[compressImages,setCompressImages]=useState(true),[attachmentPreview,setAttachmentPreview]=useState<{src:string;name:string}|null>(null),[uploading,setUploading]=useState(false),[chatSearchOpen,setChatSearchOpen]=useState(false),[chatSearch,setChatSearch]=useState("");
 ...
 const [notificationsEnabled,setNotificationsEnabled]=useState(true),[soundEnabled,setSoundEnabled]=useState(true),[clickSoundEnabled,setClickSoundEnabled]=useState(true),[updateInfo,setUpdateInfo]=useState<UpdateInfo|null>(null),[checkingUpdate,setCheckingUpdate]=useState(false);
 ...
 const chatSearchResults=useMemo(()=>{const words=chatSearch.trim().toLocaleLowerCase("ru-RU").split(/\s+/).filter(Boolean);if(!words.length)return [];return messages.filter(message=>{const text=`${message.body||""} ${message.fileName||""}`.toLocaleLowerCase("ru-RU");return words.every(word=>text.includes(word))}).slice(-30).reverse()},[messages,chatSearch]);
 const visibleChats=useMemo(()=>chats.filter(chat=>chatFilter==="all"||chatFilter==="unread"&&Boolean(chat.unreadCount)||chatFilter==="direct"&&chat.kind==="direct"||chatFolders.find(folder=>folder.id===chatFilter)?.chatIds.includes(chat.id)),[chats,chatFilter,chatFolders]);
 ...
 useEffect(()=>{
  ...
  setClickSoundEnabled(localStorage.getItem("orbit_click_sound")!=="off");
  try{setChatFolders(JSON.parse(localStorage.getItem("orbit_chat_folders")||"[]"))}catch{}
 ...
 useEffect(()=>{const click=(event:PointerEvent)=>{if(localStorage.getItem("orbit_click_sound")==="off"||!(event.target as HTMLElement).closest("button,[role=button],label,.person-row"))return;try{const context=new AudioContext(),osc=context.createOscillator(),gain=context.createGain();osc.type="sine";osc.frequency.value=145;gain.gain.setValueAtTime(.018,context.currentTime);gain.gain.exponentialRampToValueAtTime(.0001,context.currentTime+.035);osc.connect(gain).connect(context.destination);osc.start();osc.stop(context.currentTime+.04);window.setTimeout(()=>void context.close(),100)}catch{}};document.addEventListener("pointerdown",click,{passive:true});return()=>document.removeEventListener("pointerdown",click)},[]);
 useEffect(()=>{
 ...
 useEffect(()=>{
  const back=()=>{
   ...
   if(pinMessageChoice){setPinMessageChoice(null);return true}
   ...
   if(deleteChatsOpen){setDeleteChatsOpen(false);return true}
   if(folderEditor){setFolderEditor(false);return true}
   ...
   if(selectedChatIds.size){setSelectedChatIds(new Set());return true}
 ...
 },[messageMenu,pinMessageChoice,greetingOffer,aiMenuOpen,emojiOpen,attachmentOpen,deleteMessage,batchDeleteOpen,deleteChatsOpen,folderEditor,forwardMessages.length,richAttachment,composeOpen,profileOpen,privacyOpen,notificationSettingsOpen,devicesOpen,updateInfo,galleryOpen,avatarEditor,selectedMessageIds.size,selectedChatIds.size,editingMessage,replyingTo,mobileChatOpen,section]);
 useEffect(()=>{
 ...
 async function checkNotifications(){
  ...
  const latest=unread[0],isMessage=["message","file","poll","checklist"].includes(latest.kind);
  notify(latest.body);
 ...
 async function sendStructured(kind:"location"|RichAttachmentKind,payload:unknown){
  if(!activeChat||!profile)return false;const chatId=activeChat.id,temporaryId=`pending-${crypto.randomUUID()}`,body=JSON.stringify(payload),optimistic:Message={id:temporaryId,senderId:profile.id,body,kind,deliveryStatus:"sent",createdAt:Date.now()};setMessages(current=>[...current,optimistic]);setAttachmentOpen(false);setRichAttachment(null);scrollRequest.current="smooth";
  const r=await appFetch("/api/messages",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({chatId,kind,body})}),data=await r.json().catch(()=>({}));
  if(!r.ok){setMessages(current=>current.filter(message=>message.id!==temporaryId));notify(data.error||"Вложение не отправлено");return false}if(data.message)setMessages(current=>current.map(message=>message.id===temporaryId?{...data.message,deliveryStatus:"delivered"}:message));window.setTimeout(()=>void loadMessages(chatId),220);return true
 }
 ...
 async function structuredAction(message:Message,action:"poll-vote"|"checklist-toggle",index:number){
  if(message.id.startsWith("pending-"))return;const previous=message.body||"{}";try{const payload=JSON.parse(previous) as {options?:Array<{text:string;voters?:string[]}>;items?:Array<{text:string;checkedBy?:string[]}>};if(action==="poll-vote"&&payload.options?.[index]){const selected=payload.options[index].voters?.includes(profile?.id||"");payload.options=payload.options.map((option,itemIndex)=>({...option,voters:(option.voters||[]).filter(id=>id!==profile?.id).concat(itemIndex===index&&!selected&&profile?.id?[profile.id]:[])}))}else if(action==="checklist-toggle"&&payload.items?.[index]){const item=payload.items[index],checked=item.checkedBy?.includes(profile?.id||"");payload.items[index]={...item,checkedBy:checked?(item.checkedBy||[]).filter(id=>id!==profile?.id):profile?.id?[...(item.checkedBy||[]),profile.id]:item.checkedBy}}const nextBody=JSON.stringify(payload);setMessages(current=>current.map(item=>item.id===message.id?{...item,body:nextBody}:item));playStructuredClick(action==="checklist-toggle")}catch{}
  const body=action==="poll-vote"?{action,messageId:message.id,optionIndex:index}:{action,messageId:message.id,itemIndex:index};const r=await appFetch("/api/messages",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)}),data=await r.json().catch(()=>({}));if(!r.ok){setMessages(current=>current.map(item=>item.id===message.id?{...item,body:previous}:item));notify(data.error||"Не удалось обновить вложение");return}if(data.body)setMessages(current=>current.map(item=>item.id===message.id?{...item,body:data.body}:item))
 }
 function playStructuredClick(completed:boolean){if(!soundEnabled)return;try{const context=new AudioContext(),gain=context.createGain();gain.gain.setValueAtTime(.045,context.currentTime);gain.gain.exponentialRampToValueAtTime(.0001,context.currentTime+.16);gain.connect(context.destination);[completed?430:330,completed?620:440].forEach((frequency,index)=>{const oscillator=context.createOscillator();oscillator.type="sine";oscillator.frequency.value=frequency;oscillator.connect(gain);oscillator.start(context.currentTime+index*.055);oscillator.stop(context.currentTime+.18)});window.setTimeout(()=>void context.close(),260)}catch{}}
 async function reactMessage(message:Message,emoji:string){
  if(message.id.startsWith("upload-")||message.id.startsWith("pending-"))return;const mine=(message.reactions||[]).filter(item=>item.userId===profile?.id),existing=mine.some(item=>item.emoji===emoji);if(!existing&&mine.length>=2){notify("Можно выбрать не более двух реакций");return}const reaction:MessageReaction={messageId:message.id,userId:profile?.id||"",emoji,createdAt:Date.now()};setMessages(current=>current.map(item=>item.id!==message.id?item:{...item,reactions:existing?(item.reactions||[]).filter(value=>!(value.userId===profile?.id&&value.emoji===emoji)):[...(item.reactions||[]),reaction]}));const r=await appFetch("/api/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"react",chatId:activeChat?.id,messageId:message.id,emoji})});if(!r.ok){const data=await r.json().catch(()=>({}));notify(data.error||"Не удалось поставить реакцию");if(activeChat)void loadMessages(activeChat.id)}
 }
 ...
 async function messageAction(action:"reply"|"view-reply"|"copy"|"copy-link"|"share"|"forward"|"pin"|"delete",message:Message){
  ...
  if(action==="share"){
   ...
   const native=(window as typeof window&{Capacitor?:{Plugins?:{PhoneContacts?:{shareText?:(value:{title:string;text:string})=>Promise<void>}}}}).Capacitor?.Plugins?.PhoneContacts;if(native?.shareText){await native.shareText({title:"Orbit Connect",text}).catch(()=>setShareFallback({text,url:location.href}));return}if(navigator.share)await navigator.share(shareData).catch(error=>{if(!(error instanceof DOMException&&error.name==="AbortError"))setShareFallback({text,url:location.href})});else setShareFallback({text,url:location.href});
   return;
  }
  if(action==="forward"){setForwardMessages([message]);return}
  if(action==="pin"&&activeChat){if(message.pinnedAt){await pinMessage(message,"self")}else setPinMessageChoice(message);return}
  setDeleteMessage(message);
 }
 async function pinMessage(message:Message,scope:"self"|"all"){
  if(!activeChat)return;setPinMessageChoice(null);const previous=message.pinnedAt||null,optimistic=previous?null:Date.now();setMessages(current=>current.map(item=>item.id===message.id?{...item,pinnedAt:optimistic}:item));const r=await appFetch("/api/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"pin-message",chatId:activeChat.id,messageId:message.id,scope})}),data=await r.json().catch(()=>({}));if(!r.ok){setMessages(current=>current.map(item=>item.id===message.id?{...item,pinnedAt:previous}:item));notify(data.error||"Не удалось закрепить");return}setMessages(current=>current.map(item=>item.id===message.id?{...item,pinnedAt:data.pinned?data.createdAt:null}:item));notify(data.pinned?(scope==="all"?"Закреплено для всех":"Закреплено только для вас"):"Сообщение откреплено")
 }
 function toggleChatSelection(chatId:string){setSelectedChatIds(current=>{const next=new Set(current);if(next.has(chatId))next.delete(chatId);else next.add(chatId);return next})}
 async function pinSelectedChats(){for(const id of selectedChatIds){const chat=chats.find(item=>item.id===id);if(chat&&!chat.pinnedAt&&!chat.systemPinned)await togglePin(chat)}setSelectedChatIds(new Set())}
 async function removeSelectedChats(){const ids=[...selectedChatIds];const r=await appFetch("/api/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"delete-chats",chatIds:ids})}),data=await r.json().catch(()=>({}));if(!r.ok){notify(data.error||"Не удалось удалить чаты");return}const removed=new Set<string>(data.removed||[]);setChats(current=>current.filter(chat=>!removed.has(chat.id)));if(activeChat&&removed.has(activeChat.id)){setActiveChat(null);setMobileChatOpen(false)}setSelectedChatIds(new Set());setDeleteChatsOpen(false);notify(`Удалено чатов: ${removed.size}`)}
 function createChatFolder(){const name=folderName.trim();if(!name){notify("Введите название папки");return}const next=[...chatFolders,{id:`folder-${Date.now()}`,name,icon:folderIcon||"📁",chatIds:[...selectedChatIds]}];setChatFolders(next);localStorage.setItem("orbit_chat_folders",JSON.stringify(next));setFolderEditor(false);setFolderName("");setFolderIcon("📁");setSelectedChatIds(new Set());setChatFilter(next.at(-1)!.id)}
 ...
 return <main className="orbit-v4" data-theme={theme} style={{"--accent":accent,"--accent-ink":"#07100b"} as CSSProperties}>
  ...
  <section className="orbit-list">
   {selectedChatIds.size?<div className="chat-selection-bar"><button aria-label="Отменить выбор" onClick={()=>setSelectedChatIds(new Set())}>×</button><b>{selectedChatIds.size}</b><span>чатов</span><button aria-label="Закрепить выбранные" title="Закрепить" onClick={()=>void pinSelectedChats()}><OrbitIcon name="pin"/></button><button aria-label="Добавить в папку" title="Новая папка" onClick={()=>setFolderEditor(true)}><OrbitIcon name="folder"/></button><button className="danger" aria-label="Удалить выбранные" title="Удалить" onClick={()=>setDeleteChatsOpen(true)}><OrbitIcon name="delete"/></button></div>:<header><div><small>ORBIT / CONNECT <i className={`connection-mini${connectionOnline?" online":" offline"}`}>{connectionOnline?"● подключено":"● нет соединения"}</i></small><h1>{section==="chats"?"Сообщения":section==="contacts"?"Контакты":"Настройки"}</h1></div>{section!=="settings"&&<button className="compose" aria-label="Создать сообщение" onClick={()=>{setComposeOpen(true);setSearch("")}}><OrbitIcon name="compose"/></button>}</header>}
  {section==="chats"&&<><div className="chat-filter-orbit"><button className={chatFilter==="all"?"active":""} onClick={()=>setChatFilter("all")}>Все</button><button className={chatFilter==="unread"?"active":""} onClick={()=>setChatFilter("unread")}>Новые</button><button className={chatFilter==="direct"?"active":""} onClick={()=>setChatFilter("direct")}>Личные</button>{chatFolders.map(folder=><button key={folder.id} className={chatFilter===folder.id?"active":""} title={folder.name} aria-label={folder.name} onDoubleClick={()=>setChatFilter(folder.id)} onClick={()=>setChatFilter(folder.id)}>{folder.icon}</button>)}<button aria-label="Создать папку" title="Создать папку" onClick={()=>setFolderEditor(true)}>＋</button></div><div className="list-scroll chat-list-filtered">{visibleChats.map(chat=><ChatListRow key={chat.id} chat={chat} active={activeChat?.id===chat.id} selected={selectedChatIds.has(chat.id)} selectionMode={selectedChatIds.size>0} preload={()=>void preloadMessages(chat.id)} open={()=>{activeChatIdRef.current=chat.id;messageLoadToken.current++;scrollRequest.current="auto";setSelectedMessageIds(new Set());setActiveChat(chat);setMessages(cachedMessages(chat.id));setMobileChatOpen(true);void loadMessages(chat.id)}} toggle={()=>toggleChatSelection(chat.id)}/>)}</div></>}
   ...
   {section==="settings"&&profile&&<Settings profile={profile} setProfile={setProfile} saveProfile={saveProfile} toggleSync={toggleSync} syncing={syncing} syncNow={()=>syncPhonebook(false)} avatarInput={avatarInput} uploadAvatar={uploadAvatar} openPrivacy={()=>setPrivacyOpen(true)} openDevices={()=>setDevicesOpen(true)} openSound={()=>setNotificationSettingsOpen(true)} openGallery={()=>void openGallery()} avatarGallery={avatarGallery} loadAvatarGallery={()=>void loadAvatarGallery()} avatarAction={(action,id)=>void avatarAction(action,id)} theme={theme} setTheme={value=>{setTheme(value);localStorage.setItem("orbit_theme",value)}} shareApp={()=>void shareApp()}/>}
  ...
  <section className={mobileChatOpen?"orbit-chat mobile-open":"orbit-chat"}>
   {activeChat?<>
    {selectionMode?<div className="message-selection-bar"><button className="selection-close" aria-label="Отменить выбор" title="Отменить выбор" onClick={clearMessageSelection}>×</button><b className="selection-count">{selectedMessageIds.size}</b><span>выбрано</span><div><button aria-label="Копировать выбранные" title="Копировать" onClick={()=>void copySelected()}><b className="bulk-symbol">⧉</b><small>Копировать</small></button><button aria-label="Переслать выбранные" title="Переслать" onClick={forwardSelected}><b className="bulk-symbol">↗</b><small>Переслать</small></button><button aria-label="Удалить выбранные" title="Удалить" onClick={()=>setBatchDeleteOpen(true)}><b className="bulk-symbol">♲</b><small>Удалить</small></button><button aria-label="Отправить выбранные в избранное" title="В избранное" onClick={()=>void favoriteSelected()}><b className="bulk-symbol">☆</b><small>В избранное</small></button></div></div>:<header>{!nativeShell&&<button className="mobile-chat-back" aria-label="Назад" onClick={()=>setMobileChatOpen(false)}><OrbitIcon name="back"/></button>}<button className="chat-profile-avatar" aria-label={activeChat.kind==="direct"?`Открыть профиль ${activeChat.name}`:"Аватар чата"} title={activeChat.kind==="direct"?"Открыть профиль":""} disabled={activeChat.kind!=="direct"||!activeChat.userId} onClick={()=>activeChat.userId&&void openProfileById(activeChat.userId)}><Avatar name={activeChat.name} url={activeChat.avatarUrl} preset={activeChat.avatarPreset}/></button><div><b>{activeChat.name}</b><small>{activeChat.kind==="direct"?(activeChat.online?"● в сети":activeChat.lastSeenAt?`не в сети · ${new Date(activeChat.lastSeenAt).toLocaleTimeString("ru-RU",{hour:"2-digit",minute:"2-digit"})}`:"не в сети"):activeChat.kind==="channel"?"официальный канал":"группа"}</small></div><button aria-label="Поиск по чату" title="Поиск по чату" onClick={()=>setChatSearchOpen(value=>!value)}><OrbitIcon name="search"/></button><button aria-label="Аудиозвонок" onClick={()=>notify("Аудиозвонок запускается")}><OrbitIcon name="phone"/></button><button aria-label="Видеозвонок" onClick={()=>notify("Видеозвонок запускается")}><OrbitIcon name="video"/></button></header>}
    {chatSearchOpen&&<div className="chat-search-panel"><label><OrbitIcon name="search"/><input autoFocus value={chatSearch} onChange={event=>setChatSearch(event.target.value)} placeholder="Найти смысл, фразу или файл…"/></label><button aria-label="Закрыть поиск" onClick={()=>{setChatSearchOpen(false);setChatSearch("")}}>×</button>{chatSearch&&<div>{chatSearchResults.length?chatSearchResults.map(message=><button key={message.id} onClick={()=>scrollToMessage(message.id)}><time>{new Date(message.createdAt).toLocaleDateString("ru-RU")}</time><span>{message.body||message.fileName||"Вложение"}</span></button>):<small>Совпадений в загруженной истории нет</small>}</div>}</div>}
    {pinnedMessages.length>0&&<div className="pinned-message-strip"><OrbitIcon name="pin"/><div>{pinnedMessages.map((message,index)=><button key={message.id} onClick={()=>scrollToMessage(message.id)}>{message.attachments?.[0]&&<PinnedThumbnail attachment={message.attachments[0]}/>}<b>{index+1}</b><span>{message.body||`${message.attachments?.length||1} фото`}</span></button>)}</div></div>}
    <div ref={messageScrollRef} onScroll={event=>{if(event.currentTarget.scrollTop<120)loadOlderMessages()}} className={`message-scroll${selectionMode?" selecting":""}`}>{messages.map((message,index)=><Fragment key={message.id}>{(index===0||dayKey(messages[index-1].createdAt)!==dayKey(message.createdAt))&&<div className="chat-day-divider"><span>{dayLabel(message.createdAt)}</span></div>}<MessageBubble message={message} mine={message.senderId===profile?.id} currentUserId={profile?.id||""} reply={message.replyTo?messageById.get(message.replyTo):undefined} selectionMode={selectionMode} selected={selectedMessageIds.has(message.id)} menu={()=>setMessageMenu(message)} select={()=>selectMessage(message)} toggle={()=>toggleMessage(message)} answer={()=>setReplyingTo(message)} share={()=>void messageAction("share",message)} retry={()=>void retryMessage(message)} seen={()=>markMessageSeen(message.id)} structured={(action,itemIndex)=>void structuredAction(message,action,itemIndex)}/></Fragment>)}</div>
    ...
    {pendingAttachments.length>0&&<div className="pending-attachments"><header><b>{pendingAttachments.every(item=>item.kind==="photo")?`Изображений: ${pendingAttachments.length}`:"Вложение"}</b>{pendingAttachments.every(item=>item.kind==="photo")&&<button className={`quality-button ${compressImages?"sd":"hd"}`} aria-label={compressImages?"Отправить сжатые фотографии":"Отправить фотографии без сжатия"} title={compressImages?"SD — быстрее и экономнее":"HD — исходное качество"} onClick={()=>{setCompressImages(value=>!value);notify(compressImages?"HD: фотографии без сжатия":"SD: быстрая отправка со сжатием")}}>{compressImages?"SD":"HD"}</button>}</header><div>{pendingAttachments.map(item=><article key={item.id}>{item.kind==="photo"&&item.previewUrl?<button className="pending-preview" aria-label="Открыть фотографию" onClick={()=>setAttachmentPreview({src:item.previewUrl!,name:"Предпросмотр фотографии"})}><img src={item.previewUrl} alt=""/></button>:<span className="pending-file"><OrbitIcon name="file"/></span>}{item.kind==="file"&&<small>{item.file.name}</small>}<button className="pending-remove" aria-label="Убрать вложение" onClick={()=>removePendingAttachment(item.id)}>×</button></article>)}</div></div>}
  ...
  {notificationSettingsOpen&&<NotificationSettings enabled={notificationsEnabled} sound={soundEnabled} clickSound={clickSoundEnabled} close={()=>setNotificationSettingsOpen(false)} setEnabled={value=>void setNotificationPreference(value)} setSound={setSoundPreference} setClickSound={value=>{setClickSoundEnabled(value);localStorage.setItem("orbit_click_sound",value?"on":"off")}} preview={()=>void previewPlum()}/>}
  {shareFallback&&<ShareFallback value={shareFallback} close={()=>setShareFallback(null)}/>}
  ...
  {pinMessageChoice&&<div className="modal-back" onClick={()=>setPinMessageChoice(null)}><div className="choice-modal" onClick={event=>event.stopPropagation()}><small>ЗАКРЕПЛЕНИЕ</small><h2>Кому показать?</h2><button onClick={()=>void pinMessage(pinMessageChoice,"self")}><OrbitIcon name="pin"/><span><b>Только для меня</b><small>Собеседник не увидит закрепление</small></span></button><button onClick={()=>void pinMessage(pinMessageChoice,"all")}><OrbitIcon name="contacts"/><span><b>Для всех участников</b><small>Сообщение появится наверху чата у всех</small></span></button></div></div>}
  ...
  {deleteChatsOpen&&<div className="modal-back" onClick={()=>setDeleteChatsOpen(false)}><div className="confirm-chat-delete" onClick={event=>event.stopPropagation()}><OrbitIcon name="delete"/><h2>Удалить выбранные чаты?</h2><p>Чаты исчезнут из вашего списка. Это действие нужно подтвердить.</p><div><button onClick={()=>setDeleteChatsOpen(false)}>Отмена</button><button className="danger" onClick={()=>void removeSelectedChats()}>Да, удалить</button></div></div></div>}
  {folderEditor&&<div className="modal-back" onClick={()=>setFolderEditor(false)}><div className="folder-editor" onClick={event=>event.stopPropagation()}><small>НОВАЯ ПАПКА</small><h2>Группа чатов</h2><div className="folder-icons">{"📁 💬 ⭐ ❤️ 👥 💼 🎮 🎓 🏠 🚀".split(" ").map(icon=><button key={icon} className={folderIcon===icon?"active":""} onClick={()=>setFolderIcon(icon)}>{icon}</button>)}</div><label>Название<input autoFocus maxLength={24} value={folderName} onChange={event=>setFolderName(event.target.value)} placeholder="Например, Работа"/></label><p>{selectedChatIds.size?`В папку войдёт выбранных чатов: ${selectedChatIds.size}`:"Папка будет создана пустой — чаты можно добавить позже групповым выбором."}</p><div><button onClick={()=>setFolderEditor(false)}>Отмена</button><button className="confirm" onClick={createChatFolder}>Создать</button></div></div></div>}
...
function Avatar({name,url,preset}:{name:string;url?:string|null;preset?:string|null}){
 if(!url&&preset==="__orbit_news__")return <span className="orbit-avatar orbit-system-avatar news" aria-hidden="true"><svg viewBox="0 0 48 48"><path d="M8 27c9-1 16-5 25-14v25c-9-8-16-11-25-11z"/><path d="M14 28l4 12h7l-3-15M37 18c4 4 4 10 0 14"/></svg></span>;
 if(!url&&preset==="__orbit_favorite__")return <span className="orbit-avatar orbit-system-avatar favorite" aria-hidden="true"><svg viewBox="0 0 48 48"><path d="M24 39S8 30 8 18c0-8 10-12 16-4 6-8 16-4 16 4 0 12-16 21-16 21z"/><path d="M24 10v28M17 27l7 5 7-5"/></svg></span>;
 const hue=[...name].reduce((sum,char)=>sum+char.charCodeAt(0)*7,0)%360;return <span className={`orbit-avatar${preset&&!url?" preset":""}${!preset&&!url?" initials-avatar":""}`} style={url?{backgroundImage:`url(${url})`}:({"--avatar-hue":hue} as CSSProperties)}>{url?"":preset||initials(name)}</span>
}
...
function Settings({profile,setProfile,saveProfile,toggleSync,syncing,syncNow,avatarInput,uploadAvatar,openPrivacy,openDevices,openGallery,openSound,avatarGallery,loadAvatarGallery,avatarAction,theme,setTheme,shareApp}:{profile:Profile;setProfile:(value:Profile)=>void;saveProfile:(value:Profile)=>Promise<boolean>;toggleSync:(value:boolean)=>Promise<void>;syncing:boolean;syncNow:()=>void;avatarInput:React.RefObject<HTMLInputElement|null>;uploadAvatar:(event:ChangeEvent<HTMLInputElement>)=>void;openPrivacy:()=>void;openDevices:()=>void;openGallery:()=>void;openSound:()=>void;avatarGallery:Array<{id:string;url:string;label:string}>;loadAvatarGallery:()=>void;avatarAction:(action:"select"|"delete",id:string)=>void;theme:string;setTheme:(value:string)=>void;shareApp:()=>void}){
 ...
 return <div className="settings-scroll">
  ...
  <div className="setting-block"><button className="plain-row" onClick={openDevices}><span className="settings-device-mark"><i/><span><b>Устройства</b><small>QR-вход и активные сеансы</small></span></span><b className="row-arrow">›</b></button><button className="plain-row" onClick={openSound}><span><b>Настройка звука</b><small>Сообщения, уведомления и мягкое нажатие</small></span><b className="row-arrow">›</b></button><button className="plain-row" onClick={openGallery}><span><b>Личная галерея</b><small>Все отправленные фотографии и файлы</small></span><b className="row-arrow">›</b></button><button className="plain-row" onClick={openPrivacy}><span><b>Конфиденциальность</b><small>Кто видит данные профиля</small></span><b className="row-arrow">›</b></button></div><button className="app-share-row" onClick={shareApp}><span className="app-share-icon">↗</span><span><b>Поделиться приложением</b><small>Отправить ссылку контакту или в другое приложение</small></span></button>
 </div>
}

function ChatListRow({chat,active,selected,selectionMode,preload,open,toggle}:{chat:Chat;active:boolean;selected:boolean;selectionMode:boolean;preload:()=>void;open:()=>void;toggle:()=>void}){
 const timer=useRef<number|undefined>(undefined),long=useRef(false);
 function down(){preload();long.current=false;timer.current=window.setTimeout(()=>{long.current=true;toggle();navigator.vibrate?.(28)},560)}
 function clear(){if(timer.current)window.clearTimeout(timer.current)}
 return <div className={`${active?"person-row selected chat-row":"person-row chat-row"}${selected?" chat-selected":""}`} onPointerEnter={preload} onPointerDown={down} onPointerUp={clear} onPointerCancel={clear} onPointerLeave={clear}>
  <button className="person-main" onClick={()=>{if(long.current){long.current=false;return}if(selectionMode)toggle();else open()}}><Avatar name={chat.name} url={chat.avatarUrl} preset={chat.avatarPreset}/><span><b>{chat.name}</b><small>{chat.kind==="channel"?"Канал новостей":chat.kind==="group"?"Группа":"Личный чат"}</small></span>{selectionMode&&<i className={`chat-choice${selected?" checked":""}`}>{selected?"✓":""}</i>}</button>
  {Boolean(chat.unreadCount)&&<b className="unread-count" aria-label={`Непрочитанных сообщений: ${chat.unreadCount}`}>{Math.min(999,chat.unreadCount||0)}</b>}
  {(chat.pinnedAt||chat.systemPinned)&&<span className="chat-pin active" aria-label="Чат закреплён" title="Чат закреплён"><OrbitIcon name="pin"/></span>}
 </div>
}
...
function MessageBubble({message,mine,currentUserId,reply,selectionMode,selected,menu,select,toggle,answer,share,retry,seen,structured}:{message:Message;mine:boolean;currentUserId:string;reply?:Message;selectionMode:boolean;selected:boolean;menu:()=>void;select:()=>void;toggle:()=>void;answer:()=>void;share:()=>void;retry:()=>void;seen:()=>void;structured:(action:"poll-vote"|"checklist-toggle",index:number)=>void}){
 ...
 function down(event:React.PointerEvent<HTMLDivElement>){if(message.deletedAt)return;event.currentTarget.setPointerCapture(event.pointerId);start.current={x:event.clientX,y:event.clientY};longPressed.current=false;moved.current=false;suppressClick.current=false;setDragging(true);timer.current=window.setTimeout(()=>{longPressed.current=true;suppressClick.current=true;setDragX(0);if(selectionMode)toggle();else openMenu();navigator.vibrate?.(28)},560)}
...
function AlbumViewer({items,index,setIndex,close}:{items:Array<{item:MessageAttachment;src:string}>;index:number;setIndex:(value:number)=>void;close:()=>void}){
 const current=items[index],start=useRef(0);function download(){if(!current?.src)return;const anchor=document.createElement("a");anchor.href=current.src;anchor.download=current.item.fileName||`photo-${index+1}`;document.body.appendChild(anchor);anchor.click();anchor.remove()}function finish(event:React.PointerEvent){const delta=event.clientX-start.current;if(Math.abs(delta)<45||items.length<2)return;setIndex(delta<0?(index+1)%items.length:(index-1+items.length)%items.length)}
 return createPortal(<div className="image-viewer-back album-viewer" role="dialog" aria-modal="true" onPointerDown={event=>{event.stopPropagation();if(event.target===event.currentTarget)close()}}><div className="image-viewer-card" onPointerDown={event=>{event.stopPropagation();start.current=event.clientX}} onPointerUp={finish}><header><b>{index+1} из {items.length}</b><button aria-label="Закрыть" onClick={close}>×</button></header><div className="album-viewer-stage"><img src={current?.src} alt={`Фотография ${index+1}`} draggable={false}/></div><div className="album-viewer-dots">{items.map((_,itemIndex)=><button key={itemIndex} className={itemIndex===index?"active":""} aria-label={`Фотография ${itemIndex+1}`} onClick={()=>setIndex(itemIndex)}/>)}</div><button className="image-viewer-download" onClick={download}><OrbitIcon name="download"/> Скачать</button></div></div>,document.body)
}

function PinnedThumbnail({attachment}:{attachment:MessageAttachment}){const [src,setSrc]=useState(attachment.previewUrl||"");useEffect(()=>{if(src)return;let active=true,url="";appFetch(`/api/files?attachment=${encodeURIComponent(attachment.id)}&inline=1`).then(async response=>{if(!response.ok)return;url=URL.createObjectURL(await response.blob());if(active)setSrc(url)});return()=>{active=false;if(url)URL.revokeObjectURL(url)}},[attachment.id]);return src?<img className="pinned-thumbnail" src={src} alt=""/>:<OrbitIcon name="photo"/>}
...
function RichAttachmentModal({kind,contacts,close,send}:{kind:RichAttachmentKind;contacts:Profile[];close:()=>void;send:(kind:RichAttachmentKind,payload:unknown)=>Promise<boolean>}){
 ...
 function addRow(){if(rows.length>=10)return;const nextIndex=rows.length;setRows(current=>[...current,""]);window.requestAnimationFrame(()=>rowInputs.current[nextIndex]?.focus())}
...
function MessageActions({message,currentUserId,close,act,react}:{message:Message;currentUserId:string;close:()=>void;act:(action:"reply"|"view-reply"|"copy"|"copy-link"|"share"|"forward"|"pin"|"delete")=>void;react:(emoji:string)=>void}){
 const [allReactions,setAllReactions]=useState(false),[query,setQuery]=useState(""),frequent=["👍","❤️","😂","🔥","😮","🙏"],allEmoji=useMemo(()=>{const normalized=query.trim().toLocaleLowerCase("ru-RU");return [...new Set(EMOJI_GROUPS.filter(group=>!normalized||group.name.toLocaleLowerCase("ru-RU").includes(normalized)||group.keywords.includes(normalized)).flatMap(group=>group.items))]},[query,message.id]);
 function choose(emoji:string){react(emoji);close()}
 return <div className="modal-back action-back" onClick={close}><div className={`context-menu-cluster${allReactions?" all-open":""}`} onClick={event=>event.stopPropagation()}>{allReactions?<><div className="all-reactions-menu"><label><OrbitIcon name="search"/><input autoFocus value={query} onChange={event=>setQuery(event.target.value)} placeholder="Найти реакцию по смыслу…"/></label><div>{allEmoji.map((emoji,index)=><button key={`${emoji}-${index}`} onClick={()=>choose(emoji)}>{emoji}</button>)}</div></div><button className="reaction-menu-toggle" aria-label="Вернуться к действиям" title="Вернуться к действиям" onClick={()=>setAllReactions(false)}>⌃</button></>:<><div className="message-actions"><button title="Ответить" onClick={()=>act("reply")}><OrbitIcon name="reply"/><b>Ответить</b></button>{message.replyTo&&<button title="Перейти к исходному сообщению" onClick={()=>act("view-reply")}><OrbitIcon name="back"/><b>Просмотреть ответ</b></button>}<button title="Скопировать текст" onClick={()=>act("copy")} disabled={!message.body}><OrbitIcon name="copy"/><b>Копировать</b></button><button title="Скопировать прямую ссылку" onClick={()=>act("copy-link")}><OrbitIcon name="external"/><b>Копировать ссылку</b></button><button title="Отправить в другое приложение" onClick={()=>act("share")}><OrbitIcon name="share"/><b>Поделиться</b></button><button title="Переслать в другой чат Orbit" onClick={()=>act("forward")}><OrbitIcon name="forward"/><b>Переслать</b></button><button title={message.pinnedAt?"Убрать сверху":"Показать сверху"} onClick={()=>act("pin")}><OrbitIcon name={message.pinnedAt?"pin-open":"pin"}/><b>{message.pinnedAt?"Открепить":"Закрепить"}</b></button><button title="Удалить сообщение" className="danger" onClick={()=>act("delete")}><OrbitIcon name="delete"/><b>Удалить</b></button></div><div className="context-reaction-rail" aria-label="Частые реакции">{frequent.map(emoji=><button className={(message.reactions||[]).some(item=>item.userId===currentUserId&&item.emoji===emoji)?"active":""} key={emoji} onClick={()=>choose(emoji)}>{emoji}</button>)}<button className="reaction-menu-toggle" aria-label="Все реакции" title="Все реакции" onClick={()=>setAllReactions(true)}>⌄</button></div></>}</div></div>
}
...
function NotificationSettings({enabled,sound,clickSound,close,setEnabled,setSound,setClickSound,preview}:{enabled:boolean;sound:boolean;clickSound:boolean;close:()=>void;setEnabled:(value:boolean)=>void;setSound:(value:boolean)=>void;setClickSound:(value:boolean)=>void;preview:()=>void}){
 return <div className="modal-back"><div className="privacy-modal sound-settings"><header><div><small>ЗВУКИ ORBIT</small><h2>Настройка звука</h2></div><button onClick={close}>×</button></header><label className="switch-row"><span><b>Уведомления о сообщениях</b><small>Показывать имя отправителя и текст</small></span><input type="checkbox" role="switch" checked={enabled} onChange={event=>setEnabled(event.target.checked)}/><i/></label><label className="switch-row"><span><b>Звук входящих сообщений</b><small>Мягкий фирменный сигнал «плюм»</small></span><input type="checkbox" role="switch" checked={sound} onChange={event=>setSound(event.target.checked)}/><i/></label><label className="switch-row"><span><b>Звук нажатий</b><small>Еле слышимый глухой отклик элементов</small></span><input type="checkbox" role="switch" checked={clickSound} onChange={event=>setClickSound(event.target.checked)}/><i/></label><button className="plum-preview" onClick={preview}>▶ Проверить звук сообщения</button><button className="primary-action" onClick={close}>Готово</button></div></div>
}

function ShareFallback({value,close}:{value:{text:string;url:string};close:()=>void}){const encodedText=encodeURIComponent(value.text),encodedUrl=encodeURIComponent(value.url);return <div className="modal-back" onClick={close}><div className="share-fallback" onClick={event=>event.stopPropagation()}><header><b>Поделиться сообщением</b><button onClick={close}>×</button></header><a href={`https://t.me/share/url?url=${encodedUrl}&text=${encodedText}`} target="_blank" rel="noreferrer">Telegram</a><a href={`https://wa.me/?text=${encodedText}%20${encodedUrl}`} target="_blank" rel="noreferrer">WhatsApp</a><a href={`mailto:?subject=Orbit%20Connect&body=${encodedText}%0A${encodedUrl}`}>Почта</a><button onClick={()=>{void navigator.clipboard.writeText(`${value.text}\n${value.url}`);close()}}>Копировать</button></div></div>}
