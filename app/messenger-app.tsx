...
function OrbitIcon({name,className=""}:{name:string;className?:string}){return <svg className={`orbit-glyph ${className}`} aria-hidden="true"><use href={`/orbit-ui-icons.svg#${name}`}/></svg>}
...
export default function MessengerApp(){
 ...
 useEffect(()=>{
  ...
  const backToMain=()=>{
   setComposeOpen(false);setProfileOpen(null);setPrivacyOpen(false);setNotificationSettingsOpen(false);setDevicesOpen(false);setUpdateInfo(null);setMessageMenu(null);setForwardMessages([]);setSelectedMessageIds(new Set());setBatchDeleteOpen(false);setDeleteMessage(null);setGalleryOpen(false);setAvatarEditor(null);setMobileChatOpen(false);setSection("chats");
   history.pushState({orbitMain:true},"",location.href);
  };
 ...
 async function shareApp(){
  let apkPath="/orbit-connect-v7.apk";try{const response=await fetch(`/api/version?t=${Date.now()}`,{cache:"no-store"});if(response.ok)apkPath=(await response.json()).apk?.url||apkPath}catch{}
 ...
 return <main className="orbit-v4" data-theme={theme} style={{"--accent":accent,"--accent-ink":"#07100b"} as CSSProperties}>
  ...
  <section className={mobileChatOpen?"orbit-chat mobile-open":"orbit-chat"}>
   {activeChat?<>
    ...
    {activeChat.kind==="channel"&&!activeChat.canPost?<div className="channel-readonly">Новости публикует только владелец канала</div>:<><div className="ai-row"><button className="tool-icon" aria-label="ИИ-помощник" disabled={aiWorking} onClick={()=>setAiMenuOpen(value=>!value)}><OrbitIcon name="ai"/></button><button className="tool-icon" aria-label="Эмодзи и стикеры" onClick={()=>setEmojiOpen(value=>!value)}><OrbitIcon name="emoji"/></button>{aiMenuOpen&&<div className="ai-menu"><button disabled={aiWorking} onClick={()=>void ai("generate")}><OrbitIcon name="ai"/> Написать</button><button disabled={!draft||aiWorking||Boolean(profile?.autoCorrectEnabled)} title={profile?.autoCorrectEnabled?"Исправление включено автоматически":""} onClick={()=>void ai("correct")}><OrbitIcon name="read"/> Исправить ошибки</button><button disabled={!draft||aiWorking} onClick={()=>void ai("emoji")}><OrbitIcon name="emoji"/> Расставить эмодзи</button></div>}{emojiOpen&&<EmojiPicker insert={emoji=>setDraft(value=>value+emoji)} sticker={url=>void sendSticker(url)}/>}</div>
