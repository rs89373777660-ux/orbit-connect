...
export default function MessengerApp(){
 ...
 function persistRecentMessages(chatId:string,list:Message[]){try{let recent=list.slice(-50),serialized=JSON.stringify(recent);if(serialized.length>700000){recent=recent.slice(-20);serialized=JSON.stringify(recent)}localStorage.setItem(localMessageKey(chatId),serialized)}catch{}}
 ...
 async function sendPendingAttachments(){
  if(!pendingAttachments.length||!activeChat)return;const batch=[...pendingAttachments],failed:PendingAttachment[]=[];let sent=0,captionUsed=false;setProgress("ЗАГРУЖАЕМ ВЛОЖЕНИЯ…");setUploading(true);
  const headers=new Headers(),token=localStorage.getItem("orbit_session");if(token)headers.set("authorization",`Bearer ${token}`);
  for(const item of batch){try{const file=item.kind==="photo"?await compressPhoto(item.file):item.file,form=new FormData();form.set("file",file);form.set("chatId",activeChat.id);form.set("caption",captionUsed?"":draft.trim());const r=await fetch("/api/files",{method:"POST",headers,body:form});if(!r.ok)throw new Error();sent++;captionUsed=true;if(item.previewUrl)URL.revokeObjectURL(item.previewUrl)}catch{failed.push(item)}}
  setPendingAttachments(failed);if(sent){setDraft("");scrollRequest.current="smooth";await loadMessages(activeChat.id)}notify(failed.length?`Отправлено ${sent}, не удалось ${failed.length}`:sent===1?"Вложение отправлено":`Отправлено изображений: ${sent}`);setProgress("");setUploading(false)
 }
 ...
 return <main className="orbit-v4" data-theme={theme} style={{"--accent":accent,"--accent-ink":"#07100b"} as CSSProperties}>
  ...
  <section className={mobileChatOpen?"orbit-chat mobile-open":"orbit-chat"}>
   {activeChat?<>
    ...
    {pendingAttachments.length>0&&<div className="pending-attachments"><header><b>{pendingAttachments.every(item=>item.kind==="photo")?`Изображений: ${pendingAttachments.length}`:"Вложение"}</b>{pendingAttachments.every(item=>item.kind==="photo")&&<label className="compression-toggle"><input type="checkbox" checked={compressImages} onChange={event=>setCompressImages(event.target.checked)}/><i/><span>{compressImages?"Сжимать":"Оригиналы"}</span></label>}</header><div>{pendingAttachments.map(item=><article key={item.id}>{item.kind==="photo"&&item.previewUrl?<button className="pending-preview" aria-label={`Открыть ${item.file.name}`} onClick={()=>setAttachmentPreview({src:item.previewUrl!,name:item.file.name})}><img src={item.previewUrl} alt=""/></button>:<span className="pending-file"><OrbitIcon name="file"/></span>}<small>{item.file.name}</small><button className="pending-remove" aria-label={`Убрать ${item.file.name}`} onClick={()=>removePendingAttachment(item.id)}>×</button></article>)}</div></div>}
